...
public class TdsCreator implements AstVisitor<String> {
    ...
    public String visit(GreaterThan1 greaterThan1) {
        ...
        if (!((leftIdentifier.equals(Type.int_t.toString()) && rightIdentifier.equals(Type.int_t.toString()))
                || (leftIdentifier.equals(Type.string_t.toString())
                        && rightIdentifier.equals(Type.string_t.toString())))) {
    ...
    public String visit(For_ for_) {
        ...
        var.setType(type);
        if (type == "int_t" || type == "string_t" || type == "nil_t") {
    ...
    public String visit(Identifier identifier) {
        switch (identifier.value) {
            ...
            default:
                ...
                if (var == null) {
                    System.out.println("Ligne " + identifier.line + " : Erreur : la variable " + identifier.value
                            + " n'est pas définie");
                } else {
    ...
    public String visit(Type_declaration type_declaration) {
        ...
        if (t != null) {
            System.out.println(
                    "Ligne " + type_declaration.line + " : Erreur : double déclaration du type :" + t.getName());
            return null;
        } else {
            currentBlock++;
            currentImbrication++;
            ...
            if (type != null) {
                if (type == Type.array_t.toString()) {
                    ...
                    if (t1 == null) {
                        System.out.println(
                                "Ligne " + type_declaration.line + " : Erreur : type " + type2 + " non declare");
                        currentTds = save;
                        currentImbrication--;
                        currentBlock--;
                        return null;
    ...
    public String visit(Print_ print_) {
        ...
        if (param.size() != 1) {
            System.out.println("Ligne " + print_.line + " : Erreur : print requiert 1 parametre, mais " + param.size()
                    + "paramètres ont ete donnes");
            return null;
        }
        for (Ast ast : param) {
            if (!ast.accept(this).equals(Type.string_t.toString())) {
                System.out.println("Ligne " + print_.line
                        + " : Erreur : print requiert un paramètre dont le type de retour est string");
                return null;
            }
        }
        return Type.void_t.toString();
    ...
    public String visit(Printi printi) {
        ...
        if (param.size() != 1) {
            System.out.println("Ligne " + printi.line + " : " + "Erreur : printi requiert 1 paramètre, mais "
                    + param.size() + "paramètres ont ete donnes");
            return null;
        }
        if (!expre.equals(Type.int_t.toString())) {
            System.out.println("Ligne " + printi.line + " : "
                    + "Erreur : printi requiert un paramètre dont le type de retour est int_t");
            return null;
        }
    ...
    public String visit(Negation negation) {
        ...
        if (!expre.equals(Type.int_t.toString())) {
            System.out.println("Ligne " + negation.line + " : "
                    + "Erreur : la  négation s'effectue exclusivement sur des expressions de type int_t");
        }
        return Type.int_t.toString();
    }
    ...
    public String visit(FunctionCall functionCall) {
        ...
        if (func == null) {// Vérification de l'existence de la fonction
            System.out.println(
                    "Ligne " + functionCall.line + " : " + "Erreur : la fonction " + name + " n'est pas définie");
            return null;
        } else {
            ...
            if (paramFournis.size() != listParametresAttendus.size()) {// Vérification du nombre d'arguments
                System.out.println("Ligne " + functionCall.line + " : " + "Erreur : l'appel de cette fonction requiert "
                        + listParametresAttendus.size() + " paramètre(s), mais " + paramFournis.size()
                        + " paramètre(s) ont été donné(s)");
                return null;
            } else {
                for (int i = 0; i < paramFournis.size(); i++) { // Vérification des types d'arguments
                    ...
                    if (!param.equals(listParametresAttendus.get(i))) {
                        System.out.println("Ligne " + functionCall.line + " : "
                                + "Erreur : l'appel de cette fonction requiert un argument de type "
                                + listParametresAttendus.get(i) + " à la position " + Integer.toString(i + 1)
                                + " mais un argument de type " + param + " a été donné");
                        return null;
                    }
                }
            }
    ...
    public String visit(Assignement assignement) {
        ...
        if (!leftIdentifier.equals(rightIdentifier)) {
            System.out.println("Ligne " + assignement.line + " : " + "Erreur : on ne peut pas assigner un type "
                    + rightIdentifier + " à une variable de type " + leftIdentifier);
            return null;
        }
        return Type.int_t.toString();
    }
    ...
    public String visit(IfThenElse ifthenelse) {
        ...
        if (!left.equals(Type.int_t.toString())) {
            System.out.println("Ligne " + ifthenelse.line + " : "
                    + "Erreur : la condition d'un ifthenelse doit etre de type int_t");
            return null;
        }
        String right = null;
        if (ifthenelse.right != null) {
            ...
            if (!middle.equals(right)) {
                ...
                System.out.println("Ligne " + ifthenelse.line + " : "
                        + "Erreur : la 2eme et 3eme expression du if_then_else doivent être du même type");
            }
            returType = right;
        } else {
            return Type.void_t.toString();
    ...
    public String visit(Substring_ substring) {
        ...
        if (param.size() != 3) {
            System.out.println("Ligne " + substring.line + " : " + "Erreur : substring requiert 3 parametres,  "
                    + param.size() + " ont ete donnes");
            return null;
        } else {
            ...
            for (int i = 0; i < param.size(); i++) {
                ...
                if (id.accept(this) != types[i]) {
                    System.out.println(
                            "Ligne " + substring.line + " : " + "Erreur : substring requiert un parametre de type "
                                    + types[i] + " à la position " + Integer.toString(i + 1)
                                    + " mais un parametre de type " + id.accept(this) + " a été donné");
                    return null;
                }
            }
        }
    ...
    public String visit(Concat_ concat_) {
        ...
        if (param.size() != 2) {
            System.out.println("Ligne " + concat_.line + " : " + "Erreur : concat requiert 2 parametres, mais "
                    + param.size() + " ont ete donnes");
            return null;
        } else {
            int countstring = 0;
            ...
            if (!(countstring == 2)) {
                System.out.println(
                        "Ligne " + concat_.line + " : " + "Erreur : concat requiert de 2 parametres de type string ");
                return null;
            }
        }
        return Type.string_t.toString();
    ...
    public String visit(Ord_ ord_) {
        ...
        if (param.size() != 1) {
            System.out.println("Ligne " + ord_.line + " : " + "Erreur : ord requiert 1 parametre, " + param.size()
                    + " ont ete donnes");
            return null;
        } else {
            int countstring = 0;
            ...
            if (!(countstring == 1)) {
                System.out.println(
                        "Ligne " + ord_.line + " : " + "Erreur : ord requiert de 1 paramètres de type string ");
                return null;
            }
        }
        return Type.int_t.toString();
    ...
    public String visit(Size_ size_) {
        ...
        if (param.size() != 1) {
            System.out.println("Ligne " + size_.line + " : " + "Erreur : size requiert 1 paraèetre, " + param.size()
                    + " ont été donnes");
            return null;
        } else {
            int countstring = 0;
            ...
            if (!(countstring == 1)) {
                System.out.println(
                        "Ligne " + size_.line + " : " + "Erreur : size requiert de 1 paramètres de type string ");
                return null;
            }
        }
        return Type.int_t.toString();
    ...
    public String visit(Not_ not_) {
        ...
        if (param.size() != 1) {
            System.out.println("Ligne " + not_.line + " : " + "Erreur : not requiert 1 parametre, mais " + param.size()
                    + " ont ete donnes");
            return null;
        } else {
            int countint = 0;
    ...
    public String visit(Variable_declaration variable_declaration) {
        ...
        if (var != null) {
            System.out.println("Ligne " + variable_declaration.line + " : "
                    + "Erreur : double déclaration de la variable " + var.getName());
            return null;
        }
        String type = null;
        Tds save = currentTds;
        if (variable_declaration.type != null) {
            ...
            if (t == null) {
                System.out.println(
                        "Ligne " + variable_declaration.line + " : " + "Erreur : le type " + type + " n'existe pas");
                currentTds = save;
                return null;
            }
            type = t.getName();
            ...
            if (type != type2) {
                System.out.println("Ligne " + variable_declaration.line + " : " + "Erreur : le type de la variable "
                        + name + " n'est pas le même que celui de l'expression associée");
                currentTds = save;
                return null;
            }
        } else {
    ...
    public String visit(Function_declaration function_declaration) {
        ...
        TdsFunction func = findFunc(name);
        if (func != null) {
            System.out.println("Ligne " + function_declaration.line + " : "
                    + "Erreur : double déclaration de la fonction " + func.getName());
            inFunction = false;
            return null;
        }
        currentBlock++;
        ...
        if (function_declaration.body != null) {
            ...
            if (!r.equals(returnType) && !returnType.equals(Type.void_t.toString())) {
                System.out.println("Ligne " + function_declaration.line + " : "
                        + "Erreur : la fonction doit retourner un type " + returnType);
                currentTds = save;
                inFunction = false;
                currentTds.supprimerElement(f);
    ...
    public String visit(Field field) {
        ...
        if (!currentTds.contains(name)) {
            System.out.println("Ligne " + field.line + " : " + "Erreur : le champ " + name
                    + " n'existe pas pour la structure " + currentTds.getElements().get(0).getName());
            return null;
        } else {
            ...
            if (type == null) {
                ...
                if (!currentTds.getElement(name).getType().equals(type)) {
                    System.out.println("Ligne " + field.line + " : " + "Erreur : le champ " + name
                            + " doit être de type " + currentTds.getElement(name).getType());
                    return null;
                }
            }
        }
    ...
    public String visit(Field_list field_list) {
        if (currentTds.getElements().size() - 1 != field_list.fields.size()) {
            System.out.println("Ligne " + field_list.line + " : " + "Erreur : la structure "
                    + currentTds.getElements().get(currentTds.getElements().size() - 1).getName() + " possède "
                    + Integer.toString(currentTds.getElements().size() - 1) + " champs");
            return null;
        }
    ...
    public String visit(AccessId accessId) {
        ...
        if (!(tdsType instanceof TdsType) || tdsType instanceof TdsArrayType) {
            System.out.println("Ligne " + accessId.line + " : " + "Erreur : la variable de type " + left
            ...
            if (!t.contains(right)) {
                System.out.println("Ligne " + accessId.line + " : " + "Erreur : le champ " + right
                        + " n'existe pas pour la structure " + left);
                return null;
            } else {
    ...
    public String visit(AccessIndex accessIndex) {
        ...
        if (!(tdsType instanceof TdsArrayType)) {
            System.out.println("Ligne " + accessIndex.line + " : " + "Erreur : la variable de type " + left
    ...
    public String visit(RecordDec recordDec) {
        ...
        if (type == null) {
            ...
            if (!e.getType().equals(Type.record_t.toString())) {
                System.out.println(
                        "Ligne " + recordDec.line + " : " + "Erreur : " + id + " n'est pas un type de structure");
                return null;
            } else {
                currentTds = tds;
            }
    ...
    public String visit(ArrayDec arrayDec) {
        ...
        if (type == null) {
            ...
            if (!e.getType().equals(Type.array_t.toString())) {
                System.out
                        .println("Ligne " + arrayDec.line + " : " + "Erreur : " + id + " n'est pas un type de tableau");
                return null;
            }
        }
        ...
        if (!size.equals(Type.int_t.toString())) {
            System.out
                    .println("Ligne " + arrayDec.line + " : " + "Erreur : la taille d'un array doit être de type int");
            return null;
        }
        ...
        if (!e.getBaseType().equals(base)) {
            System.out.println("Ligne " + arrayDec.line + " : " + "Erreur : le type " + base
                    + " ne correspond pas au type " + e.getBaseType() + " de " + name);
            return null;
        }
        return name;
    }
